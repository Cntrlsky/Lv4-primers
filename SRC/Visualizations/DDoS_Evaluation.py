...
X = pd.read_csv("Data/Processed/DDoS_Model_PrdIntr_X.csv")
y_true = pd.read_csv("Data/Processed/DDoS_Model_PrdIntr_Y.csv")
...
plt.savefig("Docs/DDoS_Confusion_Matrix.png") # Saves the physical file
plt.show()
...
plt.savefig("Docs/DDoS_ROC_Curve.png")
plt.show()
...
with open("Docs/DDoS_CV_Results.txt", "w") as f:
...
# We train and test on the same evaluation set just to see the majority class ratio
...
rf_acc = cv_scores.mean() # Re-using your CV score
...
with open("Docs/DDoS_Baseline_Comparison.txt", "w") as f:
